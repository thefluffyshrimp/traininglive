...
function get_progressbar($userid){
    global $DB,$USER,$CFG;
    //print_r($USER);
    $fromwhen = $USER->timecreated;
    $towhen = time();
   ...
   /* $from='1552009992';
    $to='1597855916';
*/
    $logs = use_stats_extract_logs($fromwhen, $towhen, $userid);
    $aggregate = use_stats_aggregate_logs($logs, 'module', 0, $fromwhen, $towhen);

     if($aggregate){

        $datanew = $aggregate['coursetotal'];

        $categorytime =array();
        foreach ($datanew as $key => $value) {

           $course =  $DB->get_record('course', array('id' => $key), 'id, category');

           $categorytime[] = array('categoryid' =>$course->category,'totaltime' => $value->elapsed);
        ...
        $temp = [];
            foreach($categorytime as $value) {
                //check if categoryid exists in the temp array
                if(!array_key_exists($value['categoryid'], $temp)) {
                    //if it does not exist, create it with a value of 0
                    $temp[$value['categoryid']] = 0;
                }
                //Add up the values from each categoryid
                $temp[$value['categoryid']] += $value['totaltime'];
            }

         $categorytimeline=array();
         foreach ($temp as $key => $value) {
            
            $category = $DB->get_record('course_categories',array('id'=>$key));
            $categorytimeline[] = array('categoryname' =>$category->name,'totaltime' => $value);
